type error_msg =
  ...
  | SEMANTIC_ERROR of semantic_kind
...
let string_of_msg = function
  ...
  | SEMANTIC_ERROR kind -> (
      ...
      | IDENTIFIER_DECLARED_VOID_ARRAY id ->
...
let report ?token ?pos error_kind =
  let msg = string_of_msg error_kind in
  match (pos, token) with
  | None, None -> Printf.printf "ERROR: %s\n" msg
  | None, Some t -> Printf.printf "ERROR: %s: %s\n" t msg
  | Some pos, None ->
      Printf.printf "ERROR: %s: %s\n" (Source_position.to_string pos) msg
  | Some pos, Some t ->
      Printf.printf "ERROR: %s: %s: %s\n" (Source_position.to_string pos) t msg
